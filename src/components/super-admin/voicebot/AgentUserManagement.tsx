        ...
        <div className="flex flex-col h-full">
            ...
            <div className="mb-6 p-4 bg-bg-alt rounded-2xl border border-border-subtle flex items-center gap-4">
                ...
                <div>
                    ...
                    <p className="text-sm text-text-muted">Manage users for this agent</p>
                </div>
            </div>

            {/* Tabs */}
            <div className="flex border-b border-border-subtle mb-6 shrink-0">
                <button
                    onClick={() => setActiveTab("assigned")}
                    className={`flex-1 py-3 text-sm font-bold transition-all relative ${activeTab === "assigned" ? "text-primary" : "text-text-muted hover:text-text-main"}`}
                >
                    Assigned Users ({assignedUsers.length})
                    {activeTab === "assigned" && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-primary rounded-t-full" />}
                </button>
                <button
                    onClick={() => setActiveTab("assign")}
                    className={`flex-1 py-3 text-sm font-bold transition-all relative ${activeTab === "assign" ? "text-primary" : "text-text-muted hover:text-text-main"}`}
                >
                    Assign New Users
                    {activeTab === "assign" && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-primary rounded-t-full" />}
                </button>
            </div>
                    ...
                    <div className="space-y-3 px-2">
                        {assignedUsers.length > 0 ? (
                            ...
                            <div className="py-12 text-center text-text-muted italic bg-bg-alt/20 rounded-2xl border border-dashed border-border-subtle">
                                No users assigned to this agent yet.
                            </div>
                        )}
                    </div>
                ) : (
