...
function Inbound() {
    ...
    const handleViewDetails = (call: InboundCall) => {
        // focusing on ui only, using same dummy data for each
        setSelectedCallForDetail({
            ...dummyCallDetail,
            customerNumber: call.callerNumber,
                ...
                <div className="card p-4">
                    <div className="flex flex-col sm:flex-row gap-2 mb-4">
                        <div className="flex-1 relative">
                            ...
                            <input
                                type="text"
                                placeholder="Search by client, number or agent..."
                                value={searchTerm}
                        ...
                        <table className="w-full">
                            <thead>
                                <tr className="border-b border-border-subtle">
                                    ...
                                    <th className="text-left py-4 px-3 text-xs font-semibold text-text-muted tracking-wider">Date & Time</th>
                                    <th className="text-left py-4 px-3 text-xs font-semibold text-text-muted tracking-wider">Client</th>
                                    <th className="text-left py-4 px-3 text-xs font-semibold text-text-muted tracking-wider">Caller Number</th>
                                    ...
                                    <th className="text-left py-4 px-3 text-xs font-semibold text-text-muted tracking-wider">Outcome</th>
                                    <th className="text-left py-4 px-3 text-xs font-semibold text-text-muted tracking-wider">Actions</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-border-subtle/50">
                                {dummyInboundCalls.map((call, i) => (
                                    <tr key={call.id} className="hover:bg-bg-alt/30 transition-colors">
                                        ...
                                        <td className="py-4 px-3 text-sm text-text-main font-medium">{call.dateTime}</td>
                                        <td className="py-4 px-3 text-sm text-text-muted">{call.client}</td>
                                        <td className="py-4 px-3 text-sm text-text-muted">{call.callerNumber}</td>
                                        <td className="py-4 px-3 text-sm text-text-muted">{call.agent}</td>
                                        <td className="py-4 px-3">
                                            <span className={`px-2 py-1 rounded-full text-[10px] font-bold uppercase ${call.status === 'Completed' ? 'bg-success/10 text-success' : 'bg-error/10 text-error'
                                                }`}>
                                                {call.status}
                                            </span>
                                        </td>
                                        <td className="py-4 px-3 text-sm text-text-muted">{call.duration}</td>
                                        <td className="py-4 px-3">
